...
const transactionService = {
  ...
  initSession: async (partnerName: string) => {
    ...
    try {
      ...
      const response = await axios.post<{ CK: string; SK: string }>(
        process.env.BASE_URL_V2 + "/Transaction/GetSession",
        undefined,
        {
          headers: {
            CK: `${session?.CK ?? ""}`,
  ...
  register: async (
    ...
    try {
      await axios.post(
        process.env.BASE_URL_V2 + "/Transaction/Register",
        formData,
        {
          headers: {
  ...
  registerAppointment: async (
    ...
    try {
      ...
      const response = await axios.post(
        process.env.BASE_URL_V2 + "/Transaction/RegisterAppointment",
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  createQueue: async (
    ...
    try {
      ...
      const url = process.env.BASE_URL_V2 + "/Transaction/CreateQueue";
  ...
  checkQueue: async () => {
    try {
      const response: any = await axios.post(
        process.env.BASE_URL_V2 + "/Transaction/CheckQueue",
        undefined,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  cancelQueue: async () => {
    try {
      await axios.post(
        process.env.BASE_URL_V2 + "/Transaction/CancelQueue",
        undefined,
        {
          headers: {
  ...
  rateTransaction: async (
    ...
    try {
      ...
      await axios.post(
        process.env.BASE_URL_V2 + "/Transaction/Rate",
        bodyFormData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  requestCallback: async (symptom: string) => {
    try {
      ...
      await axios.post(
        process.env.BASE_URL_V2 + "/Transaction/RequestCallBack",
        bodyFormData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  getControlStates: async () => {
    try {
      const url = process.env.BASE_URL_V2 + "/Transaction/CheckControlStates";
  ...
  getConsent: async (input: { useInsurance: boolean }) => {
    try {
      const url = process.env.BASE_URL_V2 + "/Transaction/VerifyConsent";
      const response = await axios.post(
        url,
        {
  ...
  setConsent: async (input: ISetConsentInput) => {
    try {
      const url = process.env.BASE_URL_V2 + "/Transaction/SetConsent";
      const response = await axios.post(
        url,
        {
          buCode: input.buCode,
  ...
  recordDiagnosticEvent: async (eventDescription: string) => {
    try {
      ...
      await axios.post(
        process.env.BASE_URL_V2 + "/Transaction/RecordDiagnosticEvent",
        bodyFormData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
